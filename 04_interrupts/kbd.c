...
#define I8042_STAT_REG 0x65
#define I8042_DATA_REG 0x61
#define I8042_STAT_REG_SIZE 1
...
static bool put_char(struct kbd *data, char c)
{
			 ...
			 size_t size, loff_t *offset)
{
  ...
  unsigned long copy_size = copy_from_user(data->buf, user_buffer, size);
  if (!copy_size) 
    return -EFAULT;
...
{
	dev_t dev = MKDEV(KBD_MAJOR, KBD_MINOR);
	int err = 0;

	err = register_chrdev_region(dev, KBD_NR_MINORS, MODULE_NAME);

	if (err != 0) {
		pr_err("[Fail] register_region returned %d\n", err);
		goto out;
	}

	pr_info("[Success] register_chrdev_region\n");

	/* TODO 1: request the keyboard I/O ports */
	if (!request_region(I8042_DATA_REG, I8042_DATA_REG_SIZE, MODULE_NAME) ||
	    !request_region(I8042_STAT_REG, I8042_STAT_REG_SIZE, MODULE_NAME)) {
		err = -1;
		goto out_release;
	}

	pr_info("[Success] request_region\n");
	...
	err = request_irq(I8042_KBD_IRQ, kbd_interrupt_handler, IRQF_SHARED,
			  MODULE_NAME, &devs[0]);
if (err < 0) {
		pr_err("[Fail] request_irq returned %d\n", err);
		goto out_unregister;
	}

	pr_info("[Success] request_irq\n");

	cdev_init(&devs[0].cdev, &kbd_fops);
	cdev_add(&devs[0].cdev, dev, 1);
...
out_release:
	pr_info("release_region\n");
	release_region(I8042_DATA_REG, I8042_DATA_REG_SIZE);
	release_region(I8042_STAT_REG, I8042_DATA_REG_SIZE);

out_unregister:
	pr_info("unregister_chrdev_region\n");
	free_irq(I8042_KBD_IRQ, &devs[0]);
	unregister_chrdev_region(dev, KBD_NR_MINORS);
...
static void kbd_exit(void)
{
	...
	release_region(I8042_DATA_REG, I8042_DATA_REG_SIZE);
	release_region(I8042_STAT_REG, I8042_STAT_REG_SIZE);
